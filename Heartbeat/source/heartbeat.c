...
//	UART_WriteBlocking(UART3, "AT+ROLE=1\r\n", strlen("AT\r\n"));
//	uint8_t buf[100];
//	for(int i =0; i < 4; i++)
//	UART_ReadBlocking(UART3, buf, 1);
//	UART_ReadBlocking(UART3, buf, strlen("OK\r\n"));
...
void comms_task(void * pvParameters)
{
	...
	while (true) {
		...
		while (xQueueReceive(xCommsQueue, &ev, portMAX_DELAY) == pdPASS) {
